...
** 一定要修改KEY的值为自己的，否则不能获取得到天气数据！
** 在VC6下编译C文件会有变量定义需前置的问题，将c改为cpp，然后在头文件中加extern c
**********************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
// 心知天气（www.seniverse.com）IP及端口 sixthrewufw*/
#define  WEATHER_IP_ADDR   "116.62.81.138"
#define  WEATHER_PORT	   80

#define  KEY    "SE-jTO1DDhlEVIqyL"		// 心知天气注册后，每个用户自己的一个key
...
int main(void)
{
	...
	do
	{
		...
		printf("https://blog.csdn.net/zhengnianli/article/details/95070044\n");
...
static void GetWeather(char *weather_json, char *location, Weather *result)
{
	...
	// 1 初始化操作sock需要的DLL(Winsock)
	WSAStartup(MAKEWORD(2,2),&wd);  // 第一个参数是Winsock库的版本号，如2.2
	...
	{	// #define AF_INET 2 /* internetwork:UDP, TCP, etc. Address Family */
		// #define PF_INET AF_INET /* Windows is same,but Linux, Protocol Family */
		// 套接字描述符的类型：SOCK_STREAM, SOCK_DGRAM（数据包套接字）, SOCK_RAW（原始协议接口）
		// 应用程序所使用的通信协议：IPPROTO_TCP，_UDP, _ICMP
		printf("socket error!\n");
		exit(1);
	}
	...
	// 5 组合GET请求包
	sprintf(GetRequestBuf, GET_REQUEST_PACKAGE, weather_json, KEY, location); // 后面三个是函数参数
	...
	// 9 解析天气数据并保存到结构体变量weather_data中
...
static void DisplayWeather(Weather *weather_data)
{
	printf("--------------%s此时的天气情况(www.seniverse.com)：\n",weather_data->name);
	printf("\t天气：\t%s\n",weather_data->text);		
	...
	strncpy(str1,weather_data->last_update,19);
	str1[10]=' ';
	str1[19]='\0';
	...
	printf("\t气温：\t%s-%s℃\n",weather_data->low[0],weather_data->high[0]);
	...
	printf("\t气温：\t%s-%s℃\n",weather_data->low[1],weather_data->high[1]);
	...
	printf("\t气温：\t%s-%s℃\n",weather_data->low[2],weather_data->high[2]);
...
static void printf_topic(void)
{
	...
	printf("=================HTTP天气客户端==================\n");
	printf("请输入要查询天气的城市名称的拼音（如：writef）：");
}